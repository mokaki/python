...
DL00 = str(DL01+'88dbDate-3D立體打印'+time.strftime('%Y%m')+DL02)				#文件名	88dbDate-202009.txt
fp = open(DL00, "a", encoding="utf-8" )							#開/創文件
fp.writelines('88dbDate-3D立體打印'+time.strftime('%H%M%S')+'\n')			#文件第一行
fp.close()
				...
				if d03 == [] and d04 == []:								#冇WTS冇TEL走
					...
					while True:
						if d03WTS != None :		#WTS非空
							...
							fp.close()
							print ("\n成功取得3D立體打印WTS聯絡資料",(d03WTS[36:47]))
							break
						else:			
							d04 = (browser.find_elements_by_xpath(co4))	#冇WTS找TEL
							while True:
